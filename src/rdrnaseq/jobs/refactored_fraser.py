...
def fraser_pipeline(
    ...
    cohort_id: str,
    job_attrs: dict,
    output_fds_path: dict[str, Path],  # Changed to dict to match stage output
    output_prefix: Path,  # Added to match the stage call
):
    b = get_batch()
    ...
    for sample_id, input_bam_or_cram, potential_bam_path in input_bams_or_crams:
        if isinstance(input_bam_or_cram, CramPath) and isinstance(potential_bam_path, Path):
            _, output_bam = cram_to_bam(  # Prefixed unused j with underscore
...
def fraser_count_split_reads(b, fds, bam, sample_id, cohort_id, job_attrs, output_path) -> hb.ResourceFile:
    ...
    j.command(
        ...
        mv /io/work/output/cache/splitCounts/splitCounts-{sample_id}.RDS {j.out}
        rm -rf /io/work
    """)
    )
...
def fraser_merge_split_reads(b, fds, split_counts, cohort_id, job_attrs, output_paths) -> hb.ResourceGroup:
    ...
    cache_path = '/io/work/output/cache/splitCounts'
...
def fraser_count_non_split_reads(b, fds, bam, coords, sample_id, _cohort_id, job_attrs, output_path) -> hb.ResourceFile:
    ...
    j.command(
        ...
        # Check for .h5 (FRASER 2.0) then fallback to .RDS
        TARGET_FILE=$(find /io/work/output/cache/nonSplicedCounts/ -name "nonSplicedCounts-{sample_id}.*")
...
def fraser_merge_non_split_reads(
    b, fds, non_split_counts, filtered_ranges, cohort_id, job_attrs, output_path
) -> hb.ResourceFile:
    if output_path.exists():
        return b.read_input(output_path)
    ...
    setup_lines = [f'mkdir -p /io/work/output/cache/nonSplicedCounts/{cohort_id}']
    for sid, r in non_split_counts.items():
        setup_lines.append(f'EXT=$(echo {r} | sed "s/.*\\.//")')
        setup_lines.append(f'ln -s {r} /io/work/output/cache/nonSplicedCounts/{cohort_id}/nonSplicedCounts-{sid}.$EXT')
    ...
    j.command(
        ...
        tar -czvf {j.fds_tar} -C /io/work/output/savedObjects/ FRASER_{cohort_id}/
        rm -rf /io/work
    """
        )
    )
...
def fraser_analysis(b, fds_tar, cohort_id, job_attrs, output_paths):
    ...
    j.command(
        ...
        Rscript {R_ANALYSIS} --work_dir "/io/work" --cohort_id "{cohort_id}" \\
